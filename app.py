...
def _extract_timeframe_close(price_context: Dict[str, Any], timeframe_slug: str) -> Tuple[Optional[float], Optional[str]]:
    """Best-effort lookup of the most recent close for the requested timeframe."""
...
def validate_level_break(
    ...
    latest_price = _safe_float(price_context.get("latest_price"))
    debug = {
...
def _label_support_levels(levels: List[Dict[str, Any]]) -> List[Tuple[str, float]]:
    prices: List[float] = []
    seen: Set[float] = set()
    for level in levels:
...
def _label_resistance_levels(levels: List[Dict[str, Any]]) -> List[Tuple[str, float]]:
    prices: List[float] = []
    seen: Set[float] = set()
    for level in levels:
        price_val = _safe_float(level.get("price"))
...
def compute_second_level_progress(
    ...
    s1_price = _safe_float((radar_levels.get("s1") or {}).get("price")) or s_lookup.get("S1")
    s2_price = _safe_float((radar_levels.get("s2") or {}).get("price")) or s_lookup.get("S2")
    r1_price = _safe_float((radar_levels.get("r1") or {}).get("price")) or r_lookup.get("R1")
    r2_price = _safe_float((radar_levels.get("r2") or {}).get("price")) or r_lookup.get("R2")
    ...
    if progress < ACTION_CENTER_SECOND_LEVEL_APPROACH_MIN:
    ...
    if ACTION_CENTER_SECOND_LEVEL_APPROACH_MIN <= progress <= ACTION_CENTER_SECOND_LEVEL_APPROACH_MAX:
        message = "Approaching next level - run new analysis."
        partial = ACTION_CENTER_LEVEL_BREAK_PARTIAL
    elif progress >= ACTION_CENTER_SECOND_LEVEL_NEAR_PROGRESS or (gap_pct is not None and gap_pct <= ACTION_CENTER_SECOND_LEVEL_NEAR_LEVEL_PCT):
        message = (
            "Secondary support in play - run new analysis."
            if path == "support_path"
            else "Secondary resistance in play - run new analysis."
        )
        partial = ACTION_CENTER_SECOND_LEVEL_PARTIAL
...
def generate_action_json(
    ...
    s2_price = _safe_float((radar_levels.get("s2") or {}).get("price"))
    r1_price = _safe_float((radar_levels.get("r1") or {}).get("price"))
    r2_price = _safe_float((radar_levels.get("r2") or {}).get("price"))

    proximity = compute_zone_proximity(
        latest_price,
        s1_price,
        r1_price,
        ACTION_CENTER_PROXIMITY_TOLERANCE_PCT,
    )
    ...
    timeframe_slug = next(
        (slug for slug, key in _STRATEGY_KEY_BY_TIMEFRAME.items() if key == strategy_key),
        "day",
    )
